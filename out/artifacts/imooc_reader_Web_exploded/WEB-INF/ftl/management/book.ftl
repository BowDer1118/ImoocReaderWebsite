...
<div id="dialog" style="padding: 10px;display: none">
    <form class="layui-form" >
        ...
        <div class="layui-form-item">
            <!-- 作者 -->
            <input type="text" id="author" name="author" required lay-verify="required" placeholder="請輸入作者信息"
                   autocomplete="off" class="layui-input">
        </div>
...
<script>
    ...
    var table = layui.table; //table數據表格對象
    var $ = layui.$; //jQuery
    var editor = null; //wangEditor富文本編輯器對象
    //初始化圖書列表
    table.render({
        elem: '#grdBook'  //指定div
        , id : "bookList" //數據表格id
        , toolbar: "#toolbar" //指定工具欄,包含新增添加
        , url: "/management/book/list" //數據介面
        , page: true //開啟分頁
        , cols: [[ //表頭
            ...
            , {type: 'space', title: '操作', width: '200' , templet : function(d){
                    //為每一行表格數據生成"修改"與"刪除"按鈕,並附加data-id屬性代表圖書編號
    ...
    function showUpdate(obj){
        ...
        layui.layer.open({
            ...
            title: "編輯圖書", //標題
            type: 1,
            content: $('#dialog').html(), //設置對話框內容,復制自dialog DIV
            area: ['820px', '730px'], //設置對話框寬度高度
            resize: false //是否允許調整尺寸
        })

        var bookId = $(obj).data("id"); //獲取"修改"按鈕附帶的圖書編號
        $("#dlgBook #bookId").val(bookId); //為表單隱藏域賦值,提交表單時用到

        editor = new wangEditor('#dlgBook #editor'); //初始化富文本編輯器
        ...
        $("#dlgBook #optype").val("update"); //設置當前表單提交時提交至"update"更新地址
    ...
    function showCreate(){
        ...
        editor.customConfig.uploadImgServer = '/management/book/upload';//設置圖片上傳地址
    ...
    layui.form.on('submit(btnSubmit)', function(data){
        ...
        $.post("/management/book/" + optype , formData , function(json){
            if(json.code=="0"){
                //處理成功,關閉對話框,刷新列表,提示操作成功
                layui.layer.closeAll();
                table.reload('bookList');
                layui.layer.msg('數據操作成功,圖書列表已刷新');
            }else{
                //處理失敗,提示錯誤信息
